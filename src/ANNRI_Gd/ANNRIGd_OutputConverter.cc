 ...
 */
G4ReactionProductVector* ANNRIGd_OutputConverter::ConvertToG4(
		const ReactionProductVector& products)
{
	G4ReactionProductVector* g4products = new G4ReactionProductVector();

	for(ReactionProductVector::const_iterator iProduct = products.begin(),
			iEnd = products.end(); iProduct not_eq iEnd; ++iProduct) {
		...
		g4product->SetMomentum(G4ThreeVector(
				iProduct->px_,
				iProduct->py_,
				iProduct->pz_));
