...
G4HadFinalState * SKGdNeutronHPCaptureFS::ApplyYourself(const G4HadProjectile & theTrack)
{
  G4int i;
  ...
  //thePhotons = theFinalGammas2.GetGammas();
  ...
  //G4cout<<"nPhotons = "<<nPhotons<<G4endl;
  ...
  if ( nPhotons == 1 && thePhotons->operator[](0)->GetDefinition()->GetBaryonNumber() == 0 )
  {
    ...
    G4double Q = G4IonTable::GetIonTable()->FindIon(static_cast<G4int>(theBaseZ), static_cast<G4int>(theBaseA), 0, static_cast<G4int>(theBaseZ))->GetPDGMass() + G4Neutron::Neutron()->GetPDGMass()
      - G4IonTable::GetIonTable()->FindIon(static_cast<G4int>(theBaseZ), static_cast<G4int>(theBaseA+1), 0, static_cast<G4int>(theBaseZ))->GetPDGMass();
...
#include <sstream> 
void SKGdNeutronHPCaptureFS::Init (G4double A, G4double Z, G4int M, G4String & dirName, G4String & )
{
  ...
  if ( hasExactMF6 == true )
  { 
    std::cout<<"Gadolinium-"<<sA+sM<<" has exact MF6"<<std::endl;
  ...
  std::cout<<"Gadolinium-"<<sA+sM<<" has no exact MF6"<<std::endl;
