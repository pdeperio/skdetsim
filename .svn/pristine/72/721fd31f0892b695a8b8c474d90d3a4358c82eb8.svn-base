...
// GdNeutronHPCaptureFS: Karim Zbiri's code in GLG4sim
   ...
   //SKGdNeutronHPCaptureFS * theFS = new SKGdNeutronHPCaptureFS;
...
//    G4cout <<"G4NeutronHPorLCapture::G4NeutronHPorLCapture testit "<<dirName<<G4endl;
   //G4cout <<"G4NeutronHPorLCapture:: this is the actual class I open"<<G4endl;
...
G4HadFinalState * G4NeutronHPorLCapture::ApplyYourself(const G4HadProjectile& aTrack, G4Nucleus&)
{
   ...
   G4int index = theMaterial->GetElement(0)->GetIndex();
	 ...
	 std::cout<<"Material : "<<theMaterial->GetName()<<std::endl; 
   if(n!=1)
   {
      G4int i;
      xSec = new G4double[n];
      ...
      for (i=0; i<n; i++)
      {
        ...
        G4double x = aThermalE.GetThermalEnergy(aTrack, theMaterial->GetElement(i), theMaterial->GetTemperature());
				std::cout<<"Element : "<<index<<", "<<theMaterial->GetElement(i)->GetName()<<std::endl;
    ...
    //G4cout<<"G4NeutronHPorLCapture"<<G4endl;
		//std::cout<<"# Result # Capture Element : "<<index<<", "<<theMaterial->GetElement(index)->GetName()<<std::endl;
		std::cout<<"# Result # Capture Element : "<<index<<std::endl;
...
G4int G4NeutronHPorLCapture::GetVerboseLevel() const 
{
   return G4ParticleHPManager::GetInstance()->GetVerboseLevel();
}
void G4NeutronHPorLCapture::SetVerboseLevel( G4int newValue ) 
{
   G4ParticleHPManager::GetInstance()->SetVerboseLevel(newValue);
}
void G4NeutronHPorLCapture::BuildPhysicsTable(const G4ParticleDefinition&)
{
...
void G4NeutronHPorLCapture::ModelDescription(std::ostream& outFile) const
{
