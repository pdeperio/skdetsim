 ...
 */
ANNRIGd_156GdContinuumModelV2::ANNRIGd_156GdContinuumModelV2(
		const ANNRIGd_156GdContinuumModelV2& other) :
ANNRIGd_Model("156GdContinuumV2", ANNRIGd_ModelType::Mdl156GdContinuum),
eMax_(8.536), // neutron separation energy of 156Gd in MeV
dE_(other.dE_),
instID_(sInstanceCounter), lut_(0)
{
	...
	if(other.lut_)
		lut_ = static_cast<TH2D*>(other.lut_->Clone(
				TString::Format("contModelLut_%i", instID_)));
}
 ...
 */
ANNRIGd_156GdContinuumModelV2& ANNRIGd_156GdContinuumModelV2::operator=(
		const ANNRIGd_156GdContinuumModelV2& other)
{
	TH2D* lut = static_cast<TH2D*>(other.lut_->Clone("Gd156contModelLut_0"));

	delete lut_; lut_ = lut; lut = 0;
...
ANNRIGd_156GdContinuumModelV2::DoGenerate() const
{
...
double ANNRIGd_156GdContinuumModelV2::GetGammaEnergy(double eRes) const
{
	...
	const double e2 = biny <= lut_->GetNbinsY() ?
			lut_->GetBinContent(binx, biny+1) : e1;
 ...
 */
void
ANNRIGd_156GdContinuumModelV2::Initialize(const std::string& inDataFileName)
{
